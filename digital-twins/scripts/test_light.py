# test_light.py
# Scenario: Light Load Test
# Rate: 100 msg/s | Duration: 3 minutes
...
def run():
    ...
    print(f"[Light] Starting — ~1000 msg/s for {DURATION_SECONDS}s")
    print(f"[Light] Batch size: {BATCH_SIZE}, sleep: 0.1s between batches")
    print(f"[Light] Start time: {time.strftime('%H:%M:%S')}")
    ...
    while time.time() < end_time:
        ...
        if batch_counter % 10 == 0:
            ...
            print(f"[Light] {elapsed:.0f}s — sent {total_sent} msgs, rate {rate:.0f} msg/s, errors {total_errors}")
